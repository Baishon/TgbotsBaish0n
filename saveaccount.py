...
@client.on(events.NewMessage(pattern=r"^\.savelog(?:\s+(\d+))?$"))
async def savelog(event):
    ...
    try:
        ...
        # Создаём директорию
        folder_path = os.path.join(SAVE_PATH, username if username != "Не указан" else f"user_{user_id_profile}")
...
╰┈➤ 𝙍𝙖𝙯𝙣𝙤𝙨 • 𝙆𝙈𝘽𝙋"""
        ...
        await event.reply(f"Done! Данные сохранены в Documents/KMBP/{username if username != 'Не указан' else f'user_{user_id_profile}'}")
